// Global (not project-scoped) subcontractor directory. Ported from the desktop
// app; each fn takes a &Connection. Seeded once on first run from
// resources/tp-companies-seed.json (see seed_tp_companies_if_empty).
...
/// Loads a single row by id - used by the NZBN-match flow to merge API results
/// into the existing record without clobbering fields the API doesn't supply.
pub(crate) fn get_company_by_id(conn: &Connection, id: i64) -> Result<TpCompany, String> {
    let sql = format!("SELECT {SELECT_COLUMNS} FROM tp_companies WHERE id = ?1");
    conn.query_row(&sql, params![id], row_to_company)
        .map_err(map_err)
}

pub fn list_tp_companies(conn: &Connection) -> Result<Vec<TpCompany>, String> {
...
pub fn upsert_tp_company(conn: &Connection, company: TpCompany) -> Result<TpCompany, String> {
    ...
    if company.id == 0 {
        ...
        conn.execute(
            "UPDATE tp_companies SET company = ?1, legal_name_register = ?2, nzbn = ?3, \
             ...
             trades = ?14, standard_cost_code = ?15, is_active = ?16, match_status = ?17 WHERE id = ?18",
            params![
                name,
                company.legal_name_register,
                company.nzbn,
...
pub fn reorder_tp_companies(conn: &Connection, ordered_ids: Vec<i64>) -> Result<(), String> {
...
pub fn delete_tp_company(conn: &Connection, id: i64) -> Result<(), String> {
    conn.execute("DELETE FROM tp_companies WHERE id = ?1", params![id])
        .map_err(map_err)?;
    Ok(())
}

fn insert_seed(conn: &Connection, companies: &[TpCompany]) -> rusqlite::Result<()> {
...
/// One-time seed from the bundled legacy-workbook export, if the table is still
/// empty (mirrors settings::seed_settings_if_empty).
pub fn seed_tp_companies_if_empty(conn: &Connection, seed_json: &str) -> rusqlite::Result<()> {
