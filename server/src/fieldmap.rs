// Shared access to the bundled field-map.json (AcroForm field names are the
// single source of truth). Used by CSV export (field types) and import (valid
// field-name validation). Ported from the desktop app; reads from the resources
// dir instead of resolving a Tauri bundle path.
...
/// "dropdown", "signature").
pub fn load_field_types(resources_dir: &Path) -> Result<HashMap<String, String>, String> {
    let path = resources_dir.join("field-map.json");
