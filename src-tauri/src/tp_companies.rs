...
pub struct TpCompany {
    ...
    // NULL = not checked yet, 'matched' = exact match applied, 'ambiguous' =
    // multiple/fuzzy candidates need the user to pick, 'not_found' = no
    // candidates at all, 'error' = the API call itself failed.
    #[serde(default)]
